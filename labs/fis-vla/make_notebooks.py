...
def build(path):
    ...
    # 노트북은 labs/ 에서 실행되도록 경로를 맞춘다 (tinynn import, out/ 저장)
    cells.append(md(as_source("## 준비 — 실습 폴더로 경로 맞추기\n\n"
                              "`tinynn.py` 를 불러오고 결과 그림을 `labs/out/` 에 저장하기 위해 작업 폴더를 옮긴다. "
                              "저장소 밖(구글 코랩 등)에서 열었으면 공개 저장소 "
                              "[drum-grammer/vla-study](https://github.com/drum-grammer/vla-study/tree/main/labs/fis-vla)"
    ...
    cells.append(code(as_source(
        ...
        "        'https://raw.githubusercontent.com/drum-grammer/vla-study/main/labs/fis-vla/tinynn.py', 'tinynn.py')\n"
